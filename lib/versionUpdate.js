...
function execVoltaWhich(commandName, options = {}) {
  const execFn = options.execFn || exec;
  const voltaCommand = getVoltaCommand();
  const command = `${voltaCommand} which ${commandName}`;
  return new Promise((resolve) => {
    execFn(command, { windowsHide: true }, (error, stdout) => {
      if (error) {
        resolve(null);
        return;
      }
...
async function isVoltaManagedCommand(commandName, options = {}) {
  if (!commandName) {
    debugLog("volta", "skip detect: empty command name");
    return false;
  }
  ...
  try {
    ...
    debugLog("volta", "which failed", { commandName, reason: error.message });
    return false;
  }

  const voltaPath = await (options.voltaWhichFn
    ? options.voltaWhichFn(commandName)
    : execVoltaWhich(commandName, options));
  const normalizedResolvedPath = path.normalize(resolvedPath).toLowerCase();
  const normalizedVoltaPath = voltaPath ? path.normalize(voltaPath).toLowerCase() : null;
  const isVoltaManaged = Boolean(normalizedVoltaPath);
  debugLog("volta", "detect command location", {
    commandName,
    resolvedPath,
    voltaPath,
    isVoltaManaged
  });
  return isVoltaManaged;
}
...
async function runGlobalUpdate(packageName, version, options = {}) {
  ...
  debugLog("update", "selected update strategy", {
    packageName,
    version,
    targetPackage,
    strategy: useVolta ? "volta" : "npm"
  });
